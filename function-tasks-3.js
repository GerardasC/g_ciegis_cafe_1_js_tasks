...
console.group("5. Sukurkite funkciją, kuri atspausdina pirmojo parametro viduriniają raidę");
{
  ...
  function printMiddleLetter(str) {
    const letterNr = str.charAt(Math.floor(str.length / 2));
    console.log(letterNr)
  }
  console.log('---');
...
console.group("12. Sukurkite funkciją, kuri grąžina balsių kiekį žodyje");
{
  function getNumberOfVowels(str) {
    const vowels = ['a', 'e', 'y', 'u', 'i', 'o', 'ą', 'ę', 'ė', 'į', 'ų', 'ū'];
    let result = 0;
...
console.group("13. Sukurkite funkciją, kuri grąžina priebalsių kiekį žodyje");
{
  function getNumberOfConsonants(str) {
    const notVowels = ['č', 'š', 'ž', 'q', 'w', 'r', 't', 'ę', 'p', 's', 'd', 'f', 'g', 'h', 'j', 'k', 'l', 'z', 'x', 'c', 'v', 'b', 'n', 'm'];
    let result = 0;
    for (let i = 0; i < notVowels.length; i++) {
      if (str.includes(notVowels[i])) {
        for (let j = 0; j < str.length; j++) {
          if (notVowels[i] === str.charAt(j)) {
            result += 1;
          }
        }
      }
...
console.group("22. Sukurkite funkciją, kuri ištrintų paskutinę surastą 'a' raidę žodyje ir grąžintų pakeistą žodį");
{
  ...
  console.log({
    ...
    'a12a31a23': removeLastLetterA('a12a31a23'),
  })
  console.log('---');
}
console.groupEnd();
...
console.group("25. Sukurkite funkciją, kuri pirmu parametru priimtų žodį, o antruoju - masyvą su raidėmis.");
{
  function filterLetters(str, lettersToRemove) {
    for (let i = 0; i < lettersToRemove.length; i++) {
      for (let j = 0; j < str.length; j++) {
        if (str.charAt(j) === lettersToRemove[i])
...
console.group("30. Sukurkite funkciją, kuri taiso pastraipos klaidas");
{
  ...
  function splitIntoSentences(paragraph) {
    const symbols = [".","!","?"];
    let sentenceBegins = 0
    const sentences = [];
    for (let i = 0; i < paragraph.length; i++) {
      
      if (symbols.includes(paragraph[i])) {
        const sentence = capitalizeFirstWord(paragraph.slice(sentenceBegins, i+1).trim());
...
console.group("31. Sukurkite funkciją, kuri pirmu parametru priima sakinį ir sudeda atskirus sakinio žodžius į masyvą");
{
  function splitSentence(sentence) {
    ...
    for (let i = 0; i < sentence.length; i++) {
      if (sentence[i] === " ") {
        result.push(sentence.slice(counter, i))
        counter = i;
      }
      if (sentence.length - 1 === i) {
        result.push(sentence.slice(counter, i+1));
      }
    }
    return result;
  }
